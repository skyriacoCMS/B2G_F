...
void PhotonUserData::produce( edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  */
  for(size_t i = 0; i< phoColl->size();i++){
    ...
    float phoe = pho.hadronicOverEm();
  
    //    cout<<phoe<<endl;

    pho.addUserFloat("phoe",    phoe);
...
/*
bool
PhotonUserData::isMatchedWithTrigger(const pat::Electron p, trigger::TriggerObjectCollection triggerObjects, int& index)
{
...
/*
bool PhotonUserData::passIDWP(string WP, bool isEB, float dEtaIn, float dPhiIn, float full5x5, float hoe, float d0, float dz, float ooemoop, float reliso, bool conv, int missHits){
  bool pass = false;
